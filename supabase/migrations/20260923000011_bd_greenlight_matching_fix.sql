-- BD Radar: Greenlight matching fix, found while backfilling Issues 21 to 24.
-- The same-county-same-date fallback merged different items (Coachella's ban
-- into Palm Springs' moratorium). It now applies only to the hand-entered
-- Issue 25 items it was written for. Also maps the WITHDRAWN status.
...
-- One-time repair: the old fallback merged "Tech Park at Dulles Substation"
-- (denied) into "Valley North Transmission Line", marking Valley North denied.
-- Restore Valley North to its own Issue 25 status. Re-running Issue 25 then
-- adds the Tech Park item (and AB 2383) as separate records.
update bd.projects
   set stage = 'contested', status_source = 'ACTIVE', stage_changed_at = '2026-09-15'
 where external_key = 'greenlight:valley-north-transmission-line' and stage = 'denied';
