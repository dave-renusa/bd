-- BD Radar: four sales stages.
-- Signal, Qualified, Contacted, Conversation, Proposal, Won, Lost and Watch
-- become New, Working, Closed and Watch. Won and Lost live on as
-- closed_reason ('won' or 'lost'). Postgres cannot drop enum values, so the
-- retired ones stay in the type and a check constraint keeps them unused.
-- The nightly rescore no longer moves a lead to a new stage at 70; it only
-- stamps qualified_at, which the Today page and the daily alert use.

alter type bd.lead_stage rename value 'Signal' to 'New';
alter type bd.lead_stage rename value 'Qualified' to 'Working';
alter type bd.lead_stage rename value 'Won' to 'Closed';

update bd.leads set closed_reason = coalesce(closed_reason, 'lost'), stage = 'Closed' where stage = 'Lost';
update bd.leads set closed_reason = coalesce(closed_reason, 'won') where stage = 'Closed';
update bd.leads set stage = 'Working' where stage in ('Contacted', 'Conversation', 'Proposal');
alter table bd.leads alter column stage set default 'New';
alter table bd.leads add constraint leads_stage_current
  check (stage in ('New', 'Working', 'Closed', 'Watch'));
alter table bd.leads add constraint leads_closed_reason
...
begin
  ...
  update bd.leads l set
    ...
    -- Our stage is ours to set; crossing the threshold only stamps qualified_at.
  from s
  where s.id = l.id
    -- skip unchanged leads so the nightly run stays fast
    and (l.score_breakdown is distinct from s.b
         or (l.qualified_at is null and (s.b->>'total')::int >= v_threshold));
  get diagnostics n = row_count;
  return n;
end $$;
