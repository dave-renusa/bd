-- BD Radar: Greenlight backfill guard.
-- Loading an older issue after a newer one must not roll a project or
-- jurisdiction back to an older stage. Stage updates now apply only when the
-- item's date is on or after the stage already on file.
